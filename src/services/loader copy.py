...
class LoaderService:
    ...
    def _generate_smart_chunks(self, start_slot: int, end_slot: int, enabled_loaders: list):
        """Generate chunks with unique IDs, preserving existing completed chunks."""
        # DON'T truncate - preserve existing completed chunks
        # self.clickhouse.client.command("TRUNCATE TABLE load_state_chunks")
        ...
        for loader_name in enabled_loaders:
            ...
            if loader_name == "blocks":
                ...
                for i in range(start_slot, end_slot + 1, config.CHUNK_SIZE):
                    chunk_end = min(i + config.CHUNK_SIZE - 1, end_slot)
                    chunk_id = f"{loader_name}_{i}_{chunk_end}"
                    
                    # Check if this chunk already exists and is completed
                    existing_query = """
                    SELECT status FROM load_state_chunks FINAL
                    WHERE chunk_id = {chunk_id:String}
                    """
                    existing = self.clickhouse.execute(existing_query, {"chunk_id": chunk_id})
                    ...
                    # If chunk exists but not completed, reset it to pending
                    if existing:
                        logger.info("Resetting non-completed chunk to pending", 
                                   loader=loader_name,
                                   start_slot=i,
                                   end_slot=chunk_end,
                        ...
                        VALUES ({chunk_id:String}, {start_slot:UInt64}, {end_slot:UInt64}, 
                                {loader_name:String}, 'pending', '', {current_time:DateTime}, {current_time:DateTime})
                        """
                        self.clickhouse.execute(reset_query, {
                            "chunk_id": chunk_id,
                            "start_slot": i,
                            "end_slot": chunk_end,
                ...
                if config.VALIDATOR_MODE == "all_slots":
                    ...
                    if target_slots:
                        # Group target slots into logical chunks
                        chunk_size = max(1, config.CHUNK_SIZE // 100)  # Smaller chunks for daily mode
                        for i in range(0, len(target_slots), chunk_size):
                            chunk_slots = target_slots[i:i + chunk_size]
                            chunk_start = chunk_slots[0]
                            chunk_end = chunk_slots[-1]
