...
class TransformerService:
    ...
    def _get_raw_blocks_batch(self, last_processed_slot: int, batch_size: int) -> List[dict]:
        """Get next batch of raw blocks to process (String payloads)."""
        query = """
        SELECT slot, payload 
        FROM raw_blocks 
        ...
        # Return raw data - parsers will handle JSON parsing
    ...
    def _get_raw_blocks_range(self, start_slot: int, end_slot: int) -> List[dict]:
        """Get raw blocks for a specific range (String payloads)."""
        query = """
        SELECT slot, payload 
        FROM raw_blocks 
    ...
    async def _process_batch(self, raw_blocks: List[dict]) -> int:
        """Process a batch of raw blocks with String payloads."""
        ...
        for raw_block in raw_blocks:
            slot = raw_block["slot"]
            payload = raw_block["payload"]  # This is now a String, not parsed JSON
            max_slot = max(max_slot, slot)
            
            # Process with each parser
            for parser in self.parsers:
                try:
                    # Pass the raw data to parser - it will handle JSON parsing
                    parsed_data = parser.parse({"slot": slot, "payload": payload})
