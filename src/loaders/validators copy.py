...
class ValidatorsLoader(BaseLoader):
    """Loader for validator data with support for daily snapshots or all slots."""
    ...
    def prepare_row(self, slot: int, data: Dict[str, Any]) -> Dict[str, Any]:
        """Prepare validators data for database insertion."""
        # Simplified - provide slot directly instead of relying on JSON extraction
        return {
            "slot": slot,  # Direct slot value, not materialized from JSON
    ...
    def _get_daily_target_slots(self, start_slot: int, end_slot: int) -> List[int]:
        """Get target slots for daily mode (last slot of each day)."""
        try:
